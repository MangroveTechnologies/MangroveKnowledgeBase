...
curl -H "Authorization: Bearer $TOKEN" \
  "https://api.mangrovedeveloper.ai/api/v1/strategies?skip=0&limit=10"
```
...
```bash cURL
curl -H "Authorization: Bearer $TOKEN" \
  "https://api.mangrovedeveloper.ai/api/v1/strategies/278bba9d-7f4c-476c-9dea-a1a44f1fcfba"
```

```python Python
response = requests.get(
    f"https://api.mangrovedeveloper.ai/api/v1/strategies/{strategy_id}",
    headers={"Authorization": f"Bearer {token}"}
)
...
```bash
curl -X DELETE "https://api.mangrovedeveloper.ai/api/v1/strategies/278bba9d-7f4c-476c-9dea-a1a44f1fcfba" \
  -H "Authorization: Bearer $TOKEN"
```
...
```bash
curl -X PATCH "https://api.mangrovedeveloper.ai/api/v1/strategies/278bba9d-7f4c-476c-9dea-a1a44f1fcfba/status" \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"status": "paper"}'
```
