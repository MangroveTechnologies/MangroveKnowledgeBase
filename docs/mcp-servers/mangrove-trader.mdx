...
Use the [Claude Code plugin](/plugins/claude-code) to skip the manual MCP wiring — slash commands like `/mt-leaderboard` and `/mt-stats` handle the x402 round-trip for you.
...
The full grammar is in the trader plugin's `/mt-track` command — see the [Claude Code plugin](/plugins/claude-code).
