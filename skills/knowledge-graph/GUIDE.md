...
`"indicator"` (71), `"signal"` (218) and `"technical-analysis"` (304 of 632) — legal, occasionally
...
```
find("divergence") -> 34 matches, name matches first
...
**Trap:** `status` is on the node, not the edge, and only 2 of 632 nodes are deprecated. Check it
...
```python
kg.find(under="market foundations", limit=None)                 # 133 nodes
