...
def _build_example_curl(signal_name: str, params: dict) -> str:
    ...
    return f"""curl -X POST https://api.mangrovetechnologies.ai/api/v1/execution/evaluate \\
...
def generate_catalog():
    ...
    category_order = ["Momentum", "Trend", "Volume", "Volatility", "Other"]
    ...
    # Frontmatter
    lines.append("---")
    lines.append('title: "Signal Catalog"')
    lines.append(f'description: "Complete reference for all {total} trading signals"')
    lines.append("---")
    lines.append("")

    # Intro
    lines.append(f"# Signal Catalog")
    lines.append("")
    lines.append(f"Complete reference for all **{total}** trading signals ")
    lines.append(f"({trigger_count} TRIGGER, {filter_count} FILTER) organized by category.")
    lines.append("")
    lines.append(f"See [Signals Overview](/signals/overview) for an introduction to how signals work.")
    lines.append("")

    # Summary table
    lines.append("## Summary")
    lines.append("")
    lines.append("| Name | Type | Category | Requires |")
    lines.append("|------|------|----------|----------|")
    for sig in enriched:
        requires_str = ", ".join(sig.get("requires", [])) if sig.get("requires") else "None"
        sig_type = sig.get("type", "")
        lines.append(f"| [`{sig['name']}`](#{_anchor_id(sig['name'])}) | {sig_type} | {sig['category']} | {requires_str} |")
    lines.append("")
    ...
    for cat, sigs in grouped.items():
        lines.append(f"---")
        lines.append("")
        lines.append(f"## {cat} Signals")
        lines.append("")
        lines.append(f"_{len(sigs)} signals in this category._")
        lines.append("")
        ...
        for sig in sigs:
            ...
            # Type badge and requires
            lines.append(f"**Type:** {_badge(signal_type)} &nbsp;&nbsp; **Requires:** {', '.join(requires) if requires else 'None'}")
            lines.append("")

            # Description (escape < and > so MDX doesn't parse as JSX)
            if description:
            ...
            if params:
                ...
                lines.append("")
                lines.append("| Name | Type | Range | Default | Description |")
                lines.append("|------|------|-------|---------|-------------|")
                for pname, pmeta in params.items():
                    ...
                    lines.append(f"| `{pname}` | `{ptype}` | {prange} | {pdefault} | {pdesc} |")
                lines.append("")
            else:
            ...
            lines.append(_build_example_python(signal_name, params))
            lines.append("```")
            lines.append("")
            lines.append('```bash cURL')
            lines.append(_build_example_curl(signal_name, params))
            lines.append("```")
            lines.append("")
            lines.append("</CodeGroup>")
            lines.append("")
    ...
    lines.append("---")
    lines.append("")
    lines.append(f"_Generated from docstring metadata. {total} signals total ({trigger_count} TRIGGER, {filter_count} FILTER)._")
    lines.append("")

    # Write output
    output_path = os.path.join(PROJECT_ROOT, "docs", "signals", "catalog.mdx")
